...
describe("AITUSE2316 ERC-20 Token", function () {
    ...
    let initialSupply = ethers.utils.parseUnits("2000", 18); // 2000 токенов с 18 знаками
    ...
    it("Should allow transfer of tokens", async function () {
        await aituse2316.transfer(otherAccount.address, ethers.utils.parseUnits("500", 18));
        const receiverBalance = await aituse2316.balanceOf(otherAccount.address);
        expect(receiverBalance).to.equal(ethers.utils.parseUnits("500", 18));
    });

    it("Should allow approval and transferFrom", async function () {
        await aituse2316.approve(otherAccount.address, ethers.utils.parseUnits("500", 18));
        expect(await aituse2316.allowance(owner.address, otherAccount.address))
            .to.equal(ethers.utils.parseUnits("500", 18));

        await aituse2316.connect(otherAccount).transferFrom(owner.address, otherAccount.address, ethers.utils.parseUnits("500", 18));

        expect(await aituse2316.balanceOf(otherAccount.address)).to.equal(ethers.utils.parseUnits("500", 18));
    });

    it("Should return the timestamp of the latest transaction", async function () {
        const tx = await aituse2316.transfer(otherAccount.address, ethers.utils.parseUnits("100", 18));
    ...
    it("Should revert transfer if sender has insufficient balance", async function () {
        await expect(
            aituse2316.connect(otherAccount).transfer(owner.address, ethers.utils.parseUnits("500", 18))
